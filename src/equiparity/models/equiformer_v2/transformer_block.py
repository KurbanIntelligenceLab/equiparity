...
class SO2EquivariantGraphAttention(torch.nn.Module):
    ...
    def __init__(
        ...
        if self.use_gate_act:
            self.gate_act = GateActivation(
                lmax=max(self.lmax_list), mmax=max(self.mmax_list), num_channels=self.hidden_channels
            )
        else:
            if self.use_sep_s2_act:
        ...
        self.proj = SO3_LinearV2(
            self.num_heads * self.attn_value_channels, self.output_channels, lmax=self.lmax_list[0]
        )
    ...
    def forward(self, x, atomic_numbers, edge_distance, edge_index):
        ...
        x_message = SO3_Embedding(
            0, x_target.lmax_list.copy(), x_target.num_channels * 2, device=x_target.device, dtype=x_target.dtype
        )
