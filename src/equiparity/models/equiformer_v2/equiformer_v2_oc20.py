...
# OCP imports removed - using standalone version
...
from .so3 import CoefficientMappingModule, SO3_Embedding, SO3_Grid, SO3_LinearV2, SO3_Rotation
from .transformer_block import FeedForwardNetwork, SO2EquivariantGraphAttention, TransBlockV2
...
class EquiformerV2_OC20(nn.Module):
    ...
    def generate_graph(self, data):
        """Generate graph (edge_index, distances, etc.) from atomic positions.

        Simplified version for non-PBC cases. For PBC, this would use radius_graph_pbc.
        """
        if self.otf_graph:
            # Generate edges on-the-fly using radius graph
            ...
            # Compute distances and distance vectors
            row, col = edge_index
            ...
            neighbors = torch.zeros(num_atoms, dtype=torch.long, device=data.pos.device)
            neighbors.index_add_(0, edge_index[1], torch.ones_like(edge_index[1]))

            # PBC-related outputs (zeros for non-PBC)
            cell_offsets = torch.zeros(edge_index.size(1), 3, dtype=torch.long, device=data.pos.device)
