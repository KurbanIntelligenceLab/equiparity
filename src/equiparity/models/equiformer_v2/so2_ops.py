...
class SO2_m_Convolution(torch.nn.Module):
    ...
    def __init__(self, m, sphere_channels, m_output_channels, lmax_list, mmax_list):
        ...
        self.fc = Linear(num_channels, 2 * self.m_output_channels * (num_channels // self.sphere_channels), bias=False)
...
class SO2_Convolution(torch.nn.Module):
    ...
    def forward(self, x, x_edge):
        ...
        if self.extra_m0_output_channels is not None:
            ...
            x_0 = x_0.narrow(
                -1, self.extra_m0_output_channels, (self.fc_m0.out_features - self.extra_m0_output_channels)
            )
        ...
        out_embedding = SO3_Embedding(0, x.lmax_list.copy(), self.m_output_channels, device=x.device, dtype=x.dtype)
...
class SO2_Linear(torch.nn.Module):
    ...
    def __init__(
        ...
        # SO(2) linear for m = 0
        self.fc_m0 = Linear(num_channels_m0, self.m_output_channels * (num_channels_m0 // self.sphere_channels))
        num_channels_rad = num_channels_rad + self.fc_m0.in_features
        ...
        for m in range(1, max(self.mmax_list) + 1):
            ...
            fc = Linear(num_in_channels, self.m_output_channels * (num_in_channels // self.sphere_channels), bias=False)
