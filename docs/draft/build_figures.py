...
# Display names. Dictionary KEYS stay canonical ("EquiformerV2") so every data lookup
# keeps working; only what a reader sees is abbreviated.
DISP = {"EquiformerV2": "EqV2"}


def disp(core):
    return DISP.get(core, core)
...
def align_panel_letters(fig):
    """Put every panel letter in a row at one height.

    dy in panel() is an axes fraction, so a panel whose box is shrunk by
    set_aspect("equal") carries its letter lower than its neighbours. Resolve the
    aspect with a draw, group the axes into rows by their rendered vertical centre,
    then re-place each row's letters at a single figure-fraction height keyed to the
    tallest axes in that row. x stays tied to the axes, so dx keeps its meaning.
    """
    ...
    for ax in fig.axes:
        for t in ax.texts:
            s = t.get_text().strip()
            if len(s) == 1 and s in "abcdefgh" and t.get_fontweight() == "bold":
                found.append((ax, t, ax.get_window_extent(renderer=r)))
                break

    rows = []
    for ax, t, ab in found:
        cy = 0.5 * (ab.y0 + ab.y1)
        for row in rows:
            if abs(cy - row[0]) < 0.10 * H:
                row[1].append((ax, t, ab))
                break
        else:
            rows.append([cy, [(ax, t, ab)]])

    for _, members in rows:
        y_top = max(ab.y1 for _, _, ab in members)
        pad = 0.015 * max(ab.height for _, _, ab in members)
        y_fig = (y_top + pad) / H
        for ax, t, _ in members:
            t.set_transform(blended_transform_factory(ax.transAxes, fig.transFigure))
            t.set_position((t.get_position()[0], y_fig))
            t.set_va("bottom")


def save(fig, name):
    align_panel_letters(fig)
    # metadata={"CreationDate": None} omits matplotlib's timestamp stamp, so rerunning
    # this script on unchanged inputs produces byte-identical PDFs and leaves no phantom
    # diff in `git status`. Without it every `python build_figures.py` dirties the tree.
...
def fig1():
    ...
    gs = fig.add_gridspec(1, 5, width_ratios=[1.0, 0.25, 1.15, 0.43, 1.05], wspace=0.0,
                          left=0.043, right=0.986, top=0.912, bottom=0.142)
    ...
    ax = fig.add_subplot(gs[2]); panel(ax, "b", dx=-0.05)
    ...
    # ---- c: what comes out ------------------------------------------------------------
    ax = fig.add_subplot(gs[4]); panel(ax, "c", dx=-0.22)
    ...
    ax.text(1, 5.0e-6, "structurally\nzero", ha="center", fontsize=5.8 * S, color=O3,
            fontweight="bold")
    # Two lines (a single line escapes the narrow panel), anchored from the top so the
    # block grows downward into the empty space above the SO(3) whisker max (9.59)
    # instead of upward into the "true value: exactly 0" title, which y=12.0 overlapped.
    ax.text(2, 44.0, "physically\nimpossible", ha="center", va="top", fontsize=5.8 * S,
...
def fig2():
    ...
    gs = fig.add_gridspec(1, 2, width_ratios=[1.32, 1.0], wspace=0.30,
                          left=0.086, right=0.982, top=0.924, bottom=0.148)
    ...
    ax.set_xticks(range(1, 5)); ax.set_xticklabels([disp(c) for c in cores])
    ...
    ax.legend(handles=[Line2D([], [], color=GREY, marker=CORE_MK[c], ms=2.8, lw=0.9,
                              mfc="white", mew=0.7, label=disp(c))
                       for c in ["NequIP", "Allegro", "MACE", "EquiformerV2"]],
              loc="center left", bbox_to_anchor=(0.02, 0.50), ncol=1, handletextpad=0.5,
              handlelength=1.3, borderpad=0.35, labelspacing=0.35,
              frameon=False, fontsize=5.2 * S)
    if full is None:
...
def fig3():
    ...
    ax.set_ylabel(r"$\Delta$ MAE, SO(3) $-$ O(3)")
    despine(ax)
    ...
    ax.set_yticks(ys); ax.set_yticklabels([disp(c) for c in
...
def fig4():
    ...
    ax.set_yticks(ys); ax.set_yticklabels([disp(c) for c in cores])
...
NAMED = [
    ...
    # MgO NequIP SO(3) corrected 4.9e-8 -> 4.8e-8: the seed mean in
    # results/e6_named_materials.json is 4.8474e-8, which rounds to 4.8e-8. Every other
    # cell in this block round-trips against that record. See TODO_SUBMISSION.md.
    ("MgO",         "rocksalt",   "m-3m",      True,  5.7e-8, 4.8e-8, 2.4e-8, 1.3e-8, 1.9e-7),
...
def fig5():
    ...
    # hspace widened from 0.52 to 0.70 and top raised, to open a gutter between the top row
    # and panel c that holds the figure legend clear of panel a's three-line tick labels.
    gs = fig.add_gridspec(2, 2, height_ratios=[1.0, 1.25], width_ratios=[1.35, 1.0],
                          wspace=0.42, hspace=0.70,
                          left=0.150, right=0.982, top=0.955, bottom=0.105)
    ...
    # Annotation sits in the empty upper-left of the panel and points down at the m-3m
    # group. The wrap is three short lines rather than two long ones: two lines of this
    # text are wide enough to overlap the m-3 and non-cubic bars, and the white bbox
    # would then clip data.
    ax.annotate("", xy=(0.02, 0.14), xytext=(-0.34, 0.60),
                arrowprops=dict(arrowstyle="-|>", lw=0.7, color=INK, mutation_scale=5,
                                connectionstyle="arc3,rad=-0.25"))
    ax.text(-0.56, 0.72, "rotation alone\nforbids a nonzero\nvalue here", fontsize=5.0 * S,
            color=INK, ha="left", va="center", fontweight="bold", linespacing=1.15,
            bbox=dict(boxstyle="square,pad=0.15", fc="white", ec="none"))
    ax.set_xticks(range(3)); ax.set_xticklabels(labels)
    ax.set_ylim(0, 1.32); ax.set_xlim(-0.62, 2.58)
    ax.set_ylabel("false-flag fraction,\nSO(3) arms")
    despine(ax)
    # One legend for the whole figure: the four core shades are reused for panel c's SO(3)
    # markers, and the O(3) proxy covers panel c's filled circle, so panel c carries none.
    h, l = ax.get_legend_handles_labels()
    h.append(Line2D([], [], marker="o", ms=3.4, ls="none", color=O3, mec="white", mew=0.5))
    l.append("O(3)")
    ax_a = ax   # legend is placed on the figure below, once panel c exists

    # ---- b : the guarantee is differentiable -------------------------------------------
    ax = fig.add_subplot(gs[0, 1]); panel(ax, "b", dx=-0.30)
    # Floor must sit below the smallest measured O(3) Jacobian fraction (2.1e-8);
    # 4e-8 clipped two points, contradicting the caption's "full measured range".
    ax.set_yscale("log"); ax.set_ylim(1e-8, 4.0); ax.set_xlim(0.35, 1.65)
    ...
    for y, row in zip(ys, NAMED):
        ...
        # Same per-core shades as panel a, so panel a's legend identifies these markers too
        # and panel c needs no legend of its own.
        for v, mk, cshade in zip((ne, al, ma, eq), ("o", "s", "^", "D"), shades):
            ax.plot([v], [y], marker=mk, ms=3.2, mfc="none", mec=cshade, mew=0.9, zorder=4)
    ...
    ax.text(1.2e-4, 2.0, "shaded rows, class m$\\bar{3}$m:\nrotation alone forbids a response, so\n"
                         ...
                         "O(3). EqV2, equivariant only\napproximately, fails on two",
            fontsize=5.2 * S, color=GREY, ha="center", va="center", linespacing=1.35,
            bbox=dict(boxstyle="square,pad=0.2", fc="white", ec="none"))
    # Centred on 1e-5 and re-wrapped to three lines: at two lines the block was wide enough
    # to sit over the O(3) markers of these two rows (2.7e-7, 2.8e-7) and hide them.
    ax.text(1.0e-5, 8.50, "the two non-cubic rows are the test:\n"
                          "only parity forbids it, and\n"
                          "every SO(3) arm fails",
            fontsize=5.4 * S, color=SO3, ha="center", va="center", fontweight="bold",
            bbox=dict(boxstyle="square,pad=0.2", fc="white", ec="none"))

    despine(ax)

    # One legend for the whole figure, placed in the gutter between the top row and panel c:
    # vertically centred in the gap (bottom of the top row to top of c) and horizontally
    # centred on panel c, which spans the full width. Anchored in FIGURE coordinates from the
    # realized axes positions, so it tracks any later change to the gridspec.
    fig.canvas.draw()
    pa, pc = ax_a.get_position(), ax.get_position()
    # pa.y0 is the axes box, which sits ABOVE panel a's three-line tick labels; measuring the
    # gutter from it would drop the legend onto them. Use the rendered extent of the top row
    # (labels included) as the upper edge of the gap instead.
    r = fig.canvas.get_renderer()
    top_row_bot = min(a.get_tightbbox(r).transformed(fig.transFigure.inverted()).y0
                      for a in (ax_a, fig.axes[1]))
    y_mid = (top_row_bot + pc.y1) / 2.0
    x_mid = (pc.x0 + pc.x1) / 2.0
    fig.legend(h, l, loc="center", bbox_to_anchor=(x_mid, y_mid), ncol=5, handlelength=1.0,
               handletextpad=0.4, columnspacing=0.9, frameon=False)
...
def figS3():
    ...
    ax.legend(handles=[Line2D([], [], color=SO3, marker=CORE_MK[c], ms=2.8, lw=0.9,
              ...
              loc="lower center", bbox_to_anchor=(0.5, 1.0), ncol=4, handletextpad=0.5,
              columnspacing=1.0)
    # The submission includes this panel as Supplementary Fig. 1 (\includegraphics
    # {figS1_epoch_curves} in sections/supplementary.tex), so the output filename tracks
    # the manuscript rather than this function's historical name.
    save(fig, "figS1_epoch_curves")
