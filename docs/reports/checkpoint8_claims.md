...
| D | *"E2 on BaTiO₃ shows O(3) starting at zero and SO(3) at an O(1) offset"* | Both arms start at machine zero on BaTiO₃. Cubic Pm-3̄m's rotation subgroup is **432**, which forbids a rank-3 tensor outright (claim 3), so SO(3) is forced to zero there by rotation alone — no parity involved. | E2 gained **rutile TiO₂** (P4₂/mnm 136 → P4₂nm 102), rotation subgroup **422**, which permits a rank-3 invariant. There the arms separate: at δ = 0 the O(3) arms give 1.9e-07 – 9.1e-07 (machine zero) and the SO(3) arms give 0.092 – 0.282 (both seed-averaged per core). The perovskite panels are reported with the caveat stated. |
