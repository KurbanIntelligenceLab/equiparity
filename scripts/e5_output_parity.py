"""E5 -- output-level equivariance audit of every trained piezoelectric model.
...
Run once per install profile (the two cannot share an environment):
    ...
    uv run --extra nequip python scripts/e5_output_parity.py \
        --cores nequip allegro equiformer_v2
    uv run --extra mace   python scripts/e5_output_parity.py --cores mace

Results merge into ``results/e5_output_parity.json``; render with ``--render``.
"""
...
OUT_JSON = REPO / "results" / "e5_output_parity.json"
OUT_MD = REPO / "docs" / "results" / "e5_output_parity.md"
