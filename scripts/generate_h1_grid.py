"""Expand the H-1 epoch-curve grid: the 12 SO(3) piezo runs, re-instrumented, 4 cores x 3 seeds.

Identical configuration to the headline piezoelectric SO(3) runs -- same canonical npz/split,
same hyperparameters -- run under the per-epoch OOD instrumentation added to the tensor
trainers (``ood_false_flag_history`` in metrics.json). The ``dataset`` label is
``mp_piezoelectric_epochcurve`` so the run keys are suffixed and can never shadow the released
headline runs; the domain layer, `find_piezo_runs`, and `analyze_results._is_headline` all key
off it.
...
def main() -> None:
    ...
    for core in CORES:
        for seed in SEEDS:
            ...
            runs[PROFILE[core]].append(f"configs/h1curve/{name}")
