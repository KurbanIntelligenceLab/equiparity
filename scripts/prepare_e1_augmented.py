"""E1 data: the piezoelectric training set augmented with exact-zero centrosymmetric crystals.
...
Both eval sets are subsets of the headline 2,000, so E1's numbers are directly comparable to the
main table, with zero contamination. Val and test partitions are inherited unchanged from the main
piezoelectric split, so the non-centrosymmetric test MAE stays comparable too.

    uv run --extra nequip --extra data python scripts/prepare_e1_augmented.py
"""
...
OUT_EVAL = REPO / "results/e1_eval_split.json"
