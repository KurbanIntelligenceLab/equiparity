"""F3 (MACE variant) -- the size-consistency (supercell) control, MACE core.

MACE uses a different data pipeline (mace.data.AtomicData / torch_geometric batching, not
NequIP's AtomicDataDict) and a conflicting e3nn version (0.4.4 vs NequIP/Allegro's 0.6.x), so it
runs in its own dedicated env and its own thin script; see scripts/f3_size_consistency.py for the
...
parameter values). Merges into the same results/f3_size_consistency.json under cores.mace.

    python scripts/f3_size_consistency_mace.py
"""
...
def main() -> None:
    ...
    if prior is None:
        raise SystemExit(
            "results/f3_size_consistency.json not found -- run scripts/f3_size_consistency.py "
