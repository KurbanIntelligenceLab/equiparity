"""H2 : convert the two non-empirical prevalence-audit rows (ICTP, GotenNet) to measurements.

Parity is architectural, so every probe runs at **random initialisation** (no training). Probes read
external checkouts under ``third_party/`` at pinned shas and are **not** part of the CI gate; their
numbers are committed in ``results/h2_probes.json``.
...
- **GotenNet** — needs a torch-2.5.1 + PyG-extension + lightning stack that will not co-install with
  ...
  GotenNet importable); if unavailable, a source-reasoned verdict with the barrier is recorded.
    ...
    uv run --extra nequip --extra data python scripts/h2_probe_models.py            # ICTP
    uv run --extra nequip --extra data python scripts/h2_probe_models.py \
