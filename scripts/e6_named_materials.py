"""E6 -- familiar centrosymmetric materials, and what each model predicts for them.
...
tensor, so even an SO(3) model is forced to zero on them (E7). Dropping them because they are
"boring" would hide exactly the effect E7 documents.
...
absent from the MP centrosymmetric-insulator pool entirely -- GGA puts its band gap at ~0 eV, below
the 0.1 eV insulator cut -- so silicon stands in for the diamond-structure semiconductor.
...
forbids this" rather than as a counterexample.
    ...
    uv run --extra nequip --extra data python scripts/e6_named_materials.py \\
        --cores nequip allegro equiformer_v2
    uv run --extra mace --extra data python scripts/e6_named_materials.py --cores mace
    python scripts/e6_named_materials.py --render
"""
...
OUT_MD = REPO / "docs" / "results" / "e6_named_materials.md"
...
# Cubic point groups, by proper-rotation subgroup (E7).
M3BAR = range(200, 207)
