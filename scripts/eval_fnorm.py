"""T4-2 -- convert the matched cores' piezoelectric test predictions to Fnorm and EwT.

The crystal-tensor benchmark literature reports the mean Frobenius-norm distance (Fnorm) and
the error-within-threshold rate (EwT m%: fraction of test samples with
Fnorm(error)/Fnorm(label) < m%), on the JARVIS-DFT piezoelectric set with zero tensors removed
(CEITNet paper, arXiv:2602.04323, Table 5). This computes the same two metrics for every arm on
our Materials Project test split, so Methods can state where the models land. The comparison is
indicative, not head-to-head: different source database, different split, and our split keeps
its 6 zero-norm test rows (EwT is computed over the nonzero-label rows only, matching the
benchmark's zero-removal rationale; Fnorm is reported both ways).

Run once per install profile:

    uv run --extra nequip python scripts/eval_fnorm.py --cores nequip allegro equiformer_v2
    uv run --extra mace   python scripts/eval_fnorm.py --cores mace
"""
