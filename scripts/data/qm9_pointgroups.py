...
The study claims no QM9 molecule has a point group forcing its dipole to vanish. QM9 contains
high-symmetry molecules (methane, T_d), so that claim is checked here against the actual
evaluation split rather than asserted.

Criterion. The dipole is a polar vector, so the symmetry-allowed dipoles of a molecule with point
group G are the vectors fixed by every operation of G. That subspace is the image of the Reynolds
projector

    P = (1/|G|) sum_{R in G} R,

whose rank is the number of independent dipole components symmetry permits. Rank 0 means the
dipole is forced to vanish identically. This is Neumann's principle applied to a rank-1 tensor,
and it is the same argument the manuscript uses for the rank-3 piezoelectric tensor. Inversion
symmetry is sufficient for a forced zero but not necessary: T_d contains no inversion yet still
gives rank 0.

The projector is built from operations that are verified individually, by applying each to the
molecule and requiring every atom to land on an atom of the same species within ``RESID_TOL``.
This matters: the Schoenflies label alone is not reliable here. On this dataset the analyser
labels several molecules ``D2`` while returning only the two operations of ``C2``; those molecules
carry reference dipoles of 1.6-3.0 D, which a genuine D2 would forbid. Verifying the operations
and computing the rank from them resolves the contradiction, and reproduces the reference dipole
in every case.

Reads the same raw archive, exclusion list and split as scripts/data/prepare_qm9.py, so the census
is over exactly the molecules the manuscript evaluates.

Writes results/qm9_pointgroups.json.

Run: ``uv run python scripts/data/qm9_pointgroups.py``
"""
