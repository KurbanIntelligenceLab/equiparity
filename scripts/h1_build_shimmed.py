"""H1 step 1+ : build an upstream-sourced EquiformerV2 with generate_graph relocated to the shim.
...
Each edit is recorded so "measured on upstream source at 8fe8cba" is auditable. The
reconstruction is
then checked to import, instantiate, and reproduce the shipped vendored model **bit-for-bit** on a
seeded forward — which proves both that the reconstruction is faithful and that the vendored copy
carries upstream behaviour verbatim.
"""
...
# is (old, new); old must occur exactly once. Recorded verbatim in the H1 report.
