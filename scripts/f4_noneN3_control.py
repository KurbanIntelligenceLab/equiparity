...
symmetry-constructed structure. This script therefore:
  ...
  1. Builds the SAME nine idealized centrosymmetric crystals `scripts/f3_size_consistency.py`
     uses (imported directly -- that module does not import `equiparity` at module scope, only
     inside its NequIP/Allegro builder functions, so importing `build_crystals` and
     `verify_centrosymmetric` from it keeps this script standalone in the same sense as
     `scripts/t2_backbone_probe.py`).
...
# response -- same convention as scripts/f3_size_consistency.py's NOISE_FLOOR. Flagged per
...
def periodic_mirror_probe(model: MiaoNet, atoms0: Atoms, seed: int = 5) -> dict:
    ...
    exactly the caveat `scripts/f3_size_consistency.py`'s NOISE_FLOOR handles for the
