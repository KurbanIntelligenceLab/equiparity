...
Companion to ``scripts/grids/generate_grid.py`` (same
``CORE_PARITY``/``PROFILE``/``SEEDS``/``TARGET`` tables, same YAML shape and the same
``ModelHyperparams``/``io/config.py`` loading path -- no parallel mechanism). Differences from the
headline grid, both deliberate:

1. Restricted to ``piezoelectric`` and ``elastic`` -- the two intensive-property targets where
   extensive pooling could confound the result (U0 and dipole are molecular/QM9 targets with no
   supercell notion, so a pooling control there is not meaningful).
2. Every config gets ``model.pooling: mean`` (the opt-in intensive-readout arm; the
...
automatically from the training split under the run's own pooling -- see results/pooling_arms.json
for the measured per-seed values; refitting rather than reusing the sum-pooled scale is required
because the mean-pooled target distribution differs.

    python scripts/grids/generate_grid_meanpool.py
"""
