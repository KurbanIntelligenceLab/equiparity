"""T4-1: cache frozen MACE-MP-0 per-atom features for the piezo splits + OOD set.

Runs in the main repo env with the mace extra:

    uv run --extra mace python scripts/t4_cache_mace_features.py

The backbone is the released MACE-MP-0 "small" foundation model (frozen, eval mode,
float32 on CUDA). Per-atom features are read from the DEEPEST interaction linear whose
output irreps carry l>0 (mirrors ``MACETensorModel._deepest_tensor_probe``); for this
checkpoint that is ``interactions.1.linear`` with irreps ``128x0e+128x1o+128x2e+128x3o``.
Structures are fed as ``mace.data.Configuration`` WITH cell+pbc (parity leaks otherwise).

Outputs ``results/t4/mace_mp0_<set>.npz`` with keys ``features`` (sum_atoms, D) float32,
``n_atoms`` (N,), ``ids`` (N,), and for train/val/test ``targets_irreps`` (N, 18) — the
Voigt (3, 6) rows converted to the 2x1o+1x2o+1x3o irreps basis. Metadata (feature irreps,
probe layer, checkpoint, determinism check) goes to ``results/t4/mace_mp0_meta.json``.
"""
