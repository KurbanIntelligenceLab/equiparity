"""T4-1: train a piezoelectric e3nn head on cached FROZEN-backbone features.

    uv run python scripts/t4_frozen_backbone.py --backbone mace_mp0
    uv run python scripts/t4_frozen_backbone.py --backbone esen

Backbones (features cached by scripts/t4_cache_{mace,esen}_features.py):

- ``mace_mp0`` (parity-aware): features carry the checkpoint's parity-labelled irreps
  (recorded in ``results/t4/mace_mp0_meta.json``); head is the O(3)-correct
...
Matched protocol per seed {0,1,2}: Adam lr 2e-3, no weight decay, MSE on targets normalized
by the train std (over rows with a nonzero target), 150 epochs, batch 16 structures, GPU.
Per-atom head outputs are index_add-pooled per structure. After training: val/test MAE,
OOD violation vector -> ``results/t4/<backbone>_seed<k>_ood.npy``, false-flag @ 0.01 C/m^2
+ np.logspace(-4, 0, 25) threshold curve -> merged ``results/t4_frozen_backbone.json``.
"""
...
T4_DIR = REPO / "results/t4"
OUT_JSON = REPO / "results/t4_frozen_backbone.json"
...
def train_one(
    ...
    np.save(T4_DIR / f"{backbone}_seed{seed}_ood.npy", v)
...
def main() -> None:
    ...
    meta = json.loads((T4_DIR / f"{args.backbone}_meta.json").read_text())
    sets = {
        part: CachedSet(T4_DIR / f"{args.backbone}_{part}.npz", device)
