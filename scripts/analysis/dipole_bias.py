...
(scripts/data/prepare_qm9.py). Methods states this underestimates the density-functional
magnitude; this script measures that bias directly, since every raw .xyz carries both the
reference DFT dipole magnitude mu (comment line) and the per-atom Mulliken charges (last atom
column).

Restricted to the 130,831 kept molecules (uncharacterized excluded), and cross-checked against the
stored ``dipole`` vectors in qm9_processed.npz.

    uv run python scripts/analysis/dipole_bias.py
"""
