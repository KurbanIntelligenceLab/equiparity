...
Reads the flattened per-run metrics mirror plus the per-structure OOD violation vectors, and
emits:
...
Statistical note: seed-level paired tests have n=3, so a two-sided Wilcoxon signed-rank cannot
fall below p=0.25 regardless of effect size. Seed-level p-values are therefore reported as
descriptive only; the headline OOD claim is tested at the structure level (n=2000 centrosymmetric
crystals, paired by structure), where the test has real power.
"""
...
# Side studies (the augmented piezoelectric set) share a target with the headline grid and would
...
def bootstrap_stats(vecs, tau: float = 1e-2, n_boot: int = 2000, seed: int = 0) -> dict:
    """Percentile-bootstrap 95% CIs for the false-flag fractions (bootstrap confidence intervals).
...
def write_appendices(runs, vecs, extra) -> None:
    ...
    gen = "Generated by `scripts/analysis/aggregate_grid.py`. Do not edit by hand.\n"
    (DOCS / "per_seed.md").write_text(
        f"# Per-run results (all 84 runs)\n\n{gen}\n" + per_seed_table(runs) + "\n"
    )
    (DOCS / "threshold_curves.md").write_text(
        f"# False-flag fraction vs threshold\n\n{gen}\n"
        "Mean over 3 seeds. Machine-readable form: `results/threshold_curves.csv`.\n\n"
        + threshold_tables(runs)
        + "\n"
    )
    (DOCS / "violation_distributions.md").write_text(
        f"# Violation-magnitude distributions\n\n{gen}\n"
    ...
    (DOCS / "compute.md").write_text(f"# Compute resources\n\n{gen}\n" + extra["compute_md"] + "\n")
