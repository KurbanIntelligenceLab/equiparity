"""E2 -- symmetry-breaking curves: the guarantee turns off exactly when the symmetry does.

Parameterise ``x(delta) = x_parent + delta * dx`` along a [001] polar mode. At ``delta = 0`` the
crystal is centrosymmetric and the piezoelectric tensor is exactly zero by Neumann's principle; for
every ``delta > 0`` it is polar and a response is allowed.
...
Run per profile, then render:
    ...
    uv run --extra nequip --extra data python scripts/e2_symmetry_breaking.py \\
        --cores nequip allegro equiformer_v2
    uv run --extra mace --extra data python scripts/e2_symmetry_breaking.py --cores mace
    python scripts/e2_symmetry_breaking.py --render
"""
...
OUT_CSV = REPO / "results" / "e2_symmetry_breaking.csv"
