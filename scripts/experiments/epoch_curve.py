"""False-flag fraction against training epoch, from the re-instrumented retrains.

Collects ``ood_false_flag_history`` (idealized variant, every epoch) from the 12 SO(3) epoch-curve
retrains (``dataset: mp_piezoelectric_epochcurve``) synced into the MIRROR, plus, as a bonus, the
zero-injection runs which carry the same instrumentation. Aggregates per core (mean and min-max
band over seeds) and cross-checks every endpoint against the released headline false-flag
fractions.

    uv run python scripts/experiments/epoch_curve.py
"""
...
OUT_JSON = REPO / "results" / "epoch_curve.json"
OUT_MD = REPO / "docs" / "results" / "epoch_curve.md"
...
def main() -> None:
    ...
    lines = [
        "# False-flag fraction vs training epoch (re-instrumented retrains)",
        "",
