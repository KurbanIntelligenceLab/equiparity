"""Cache frozen pretrained eSEN per-atom features for the piezoelectric splits and OOD set.

Standalone by design: runs INSIDE the isolated fairchem-1.10 venv and imports nothing from
`equiparity` (targets in the irreps basis are read from the already-cached
``results/frozen_backbone/mace_mp0_<set>.npz`` files, which the main env wrote):

    HF_TOKEN=<token> third_party/venvs/fairchem1/bin/python \
        scripts/experiments/cache_esen_features.py

CHECKPOINT ACCESS IS GATED. Every released eSEN checkpoint (esen_30m_{oam,omat,mptrj}.pt, fairchem
registry names eSEN-30M-{OAM,OMAT24,MP}) lives in the HuggingFace repo ``facebook/OMAT24`` (mirror
``fairchem/OMAT24``), which requires accepting the FAIR Chemistry License with a logged-in HF
account. Anonymous download returns 401. Steps:
...
CUDA does not work in this venv on the RTX 5090 (torch 2.4 cu121, no sm_120): runs on CPU. This
also makes the cached features bit-deterministic (verified below).

Outputs ``results/frozen_backbone/esen_<set>.npz`` with ``features`` (sum_atoms, (lmax+1)^2 * C)
float32 in coeff-(l,m)-major layout (the raw ``node_embedding`` flattened), ``n_atoms``, ``ids``,
and for train/val/test ``targets_irreps`` (N, 18). Layout (lmax, channels) and checkpoint
provenance go to ``results/frozen_backbone/esen_meta.json``.
"""
...
def main() -> None:
    ...
    meta = {
        ...
        "lmax": lmax,
        "channels": channels,
        "layout": "coeff-(l,m)-major (n, (lmax+1)^2 * channels); remap in frozen_backbone",
        "dtype": "float32",
        "device": "cpu",
