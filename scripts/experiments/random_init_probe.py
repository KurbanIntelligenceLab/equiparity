"""Random-init rank-3 structural test and reflection probe for eSEN / UMA / EquiformerV3.

Standalone by design: this script runs INSIDE the model's isolated venv (fairchem-core cannot
co-install with the repo's torch-2.11 env) and therefore imports nothing from `equiparity`. Data
comes straight from the committed npz; the (l,m)-major -> e3nn mul-major remap and the
attach-a-rank-3-head construction replicate `EquiformerV2TensorModel` exactly.

Theorem 1 holds at any parameters, so the structural test runs at RANDOM INIT: attach the standard
rank-3 head (`o3.Linear` onto all-even features, output relabeled even -- the SO(3) arm
construction) and evaluate ||T||_F on the 2,000 idealized centrosymmetric crystals. The scale-free
internal control is the point-group family split: an exactly rotation-equivariant model is forced
to machine zero on the 166 m-3m crystals (rotation ceiling) while nothing forces the others -- so
the m-3m floor vs non-cubic magnitude ratio is the structural result, independent of the random
head's arbitrary scale.

The output-parity audit-style output-level probe (mirror / rotation / determinism laws, physical
odd-parity Wigner D) runs on 25 non-centrosymmetric piezoelectric structures.

    third_party/venvs/fairchem1/bin/python scripts/experiments/random_init_probe.py --model esen
    third_party/venvs/fairchem/bin/python  scripts/experiments/random_init_probe.py --model uma
    third_party/venvs/fairchem1/bin/python scripts/experiments/random_init_probe.py --model eqv3
"""
...
OUT_JSON = REPO / "results" / "random_init.json"
...
def output_probe(model: RankThreeHead) -> dict:
    """Output-parity laws on non-centrosymmetric structures: mirror, rotation, determinism."""
    ...
    def rel(actual, expected):
        ...
        # zero, and a ratio there divides noise by noise (the output-parity caveat). Keep structures
