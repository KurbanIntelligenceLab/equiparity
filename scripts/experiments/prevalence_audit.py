"""The prevalence audit: which released models carry parity labels on their features?

This is Table 1 of the paper. Every row is classified by **inspecting released source**, and
records the version or commit inspected plus the specific line that decides the classification.
Nothing here is taken from a paper's prose; where the source does not decide the question, the row
says so.

Categories ---------- ``parity-aware``   features carry an e3nn parity label
(``0e``/``1o``/``2e``), or are built from an
                   ...
                   input.
``SO(3)-only``     features are rotation-typed but parity-free (all-even irreps, or an
``lmax``-only
...
``invariant``      the network sees only distances (and angles); it cannot express any equivariant
                   output at all.
``undetermined``   source inspection does not settle it; a reflection test on the built model
would.

    uv run --extra nequip python scripts/experiments/prevalence_audit.py
"""
...
OUT_MD = REPO / "docs" / "results" / "prevalence_audit.md"
...
ROWS: list[dict] = [
    ...
    },
    {
        ...
        "evidence": (
            ...
            "`Irreps` constructions and no parity anywhere. Confirmed behaviourally in the "
            "output-parity audit: it "
    ...
    },
    {
        ...
        "evidence": (
            ...
            "**Measured"
            "(random init, `results/inheritance_probes.json`):** "
            "`RankThreeCartesianHarmonics` gives "
    ...
    },
    {
        ...
        "evidence": (
            ...
            "(isolated CPU env, random init, `results/inheritance_probes.json`) "
            "overturns that**: the "
    ...
    # ---- The current deployed generation: 18 models ----
    {
        "model": "eSEN",
        ...
        "evidence": (
            ...
            "typing, no rank-3 path. Measured (the random-init probe, random init, rank-3 head "
            "attached): rotation "
            "law 4.2e-5, deterministic, mirror law violated 2.0; m-3m/non-cubic violation ratio "
            "1.6e-5 on the 2,000-crystal population (`results/random_init.json`)."
        ),
    },
    {
        "model": "UMA",
        ...
        "evidence": (
            ...
            "same backbone). Zero `parity` references anywhere in `models/uma/`. Measured (the "
            "random-init probe, "
            "random init): rotation law 1.0e-4, deterministic, mirror law violated 1.8; "
            "m-3m/non-cubic ratio 1.9e-5 (`results/random_init.json`)."
        ),
    },
    {
        "model": "EquiformerV3",
        ...
        "evidence": (
            ...
            "(the random-init probe, random init): rotation-exact (2.2e-5) AND deterministic -- "
            "unlike EquiformerV2 "
            "-- yet the mirror law is violated by 2.0; m-3m/non-cubic ratio 1.2e-6. This removes "
            "the approximate-equivariance confound entirely (`results/random_init.json`)."
        ),
    },
]
...
def main() -> None:
    ...
    lines += [
        ...
        "",
        "## Notes (the inheritance probes: two rows converted from reading to measurement)",
        "",
        "- `GotenNet` was `undetermined`; **now `parity-aware` by measurement** (the inheritance "
        "probes). We suspected "
        ...
        "measurement**"
        "(the inheritance probes): its rank-3 Cartesian harmonic is odd (`(-1)^3`) to machine "
        "precision, rank-2 even.",
        ...
        "- the inheritance probes converted the GotenNet and ICTP rows from source-reading to "
        "measurement; GotenNet "
        "moved"
