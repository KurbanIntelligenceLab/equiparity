"""The size-consistency (supercell) control.

The readout sums per-atom/per-edge contributions (index_add_ in every core, see the module
docstrings in equiparity.models.{nequip,allegro}), so the predicted tensor is extensive where the
piezoelectric/elastic tensor is intensive: predictions on a primitive cell and on a supercell of
the SAME crystal are not guaranteed to agree. If they disagree, extensive pooling rather than
parity labelling could account for the separation. This script tests it.
...
scripts/experiments/random_init_probe.py and scripts/experiments/per_atom_readout.py already rely
on.
...
split data/splits/*.npz are committed; the raw MP fetch requires network access to the Materials
Project API). This script therefore builds idealized centrosymmetric test crystals directly with
ase.spacegroup.crystal -- exactly space-group-symmetric coordinates, the same "idealized"
convention scripts/data/idealize_ood.py applies to the MP-derived OOD set (idealize_ood.py snaps
raw MP coordinates onto the space group because "the piezoelectric tensor is exactly zero for the
IDEAL centrosymmetric crystal"). Nine crystals spanning six distinct space groups (225, 221, 227,
136, 141, 167) and the point groups already used as the OOD family split (m-3m, non-cubic) are
ample: this is a deterministic algebraic property of the readout (sum vs. mean over an
index_add_), not a statistic that needs a large sample to resolve.
...
mean pooling is exactly invariant.
    ...
    python scripts/experiments/size_consistency.py                 # NequIP + Allegro
    python scripts/experiments/size_consistency.py --core nequip    # one core only
"""
...
def build_crystals() -> dict[str, dict]:
    ...
    by construction, the same "idealized" convention as scripts/data/idealize_ood.py)."""
    specs = {
        "NaCl_rocksalt": dict(
            symbols=["Na", "Cl"],
...
# machine-precision noise floor (the cubic "rotation ceiling" from random_init_probe.py -- an
    ...
    results: dict = {
        "provenance": {
            ...
            "reason": (
                ...
                "scripts/data/prepare_mp.py); built idealized centrosymmetric crystals directly "
                "with ase.spacegroup.crystal instead, matching the 'idealized' convention "
                "scripts/data/idealize_ood.py applies to the MP-derived OOD set"
            ),
