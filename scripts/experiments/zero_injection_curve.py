"""The zero-injection learning curve: false-flag fraction against how many exact zeros are trained
on.

For N_zero in {0, 250, 1000, 4000, 16000}: false-flag fraction and violation median on the
held-out 2,000 (global + SEEN-SG + UNSEEN-SG partitions from
``results/augmentation_eval_split.json``), non-centrosymmetric test MAE, and (with ``--control``,
GPU) the in-training control -- the false-flag fraction on the very crystals trained on with
explicit zero labels, via checkpoint reload exactly as ``augmentation.py::in_train_control``.

N = 0 reuses the headline NequIP SO(3) runs; N = 1000 the augmented runs; the
rest come from the zero-injection curve grid (datasets ``mp_piezoelectric_augmented_n{N}``). All
sets are nested.

Title rule (prespecified, applied neutrally): if the curve plateaus well above zero, "Not all
symmetry can be learned" survives; if it falls towards zero, revert to "Rotation is not enough..."
and reframe the augmentation study as cost-of-data.

    uv run --extra nequip python scripts/experiments/zero_injection_curve.py [--control]
"""
...
EVAL_SPLIT = REPO / "results" / "augmentation_eval_split.json"
OUT_JSON = REPO / "results" / "zero_injection_curve.json"
OUT_MD = REPO / "docs" / "results" / "zero_injection_curve.md"
...
def main() -> None:
    ...
    if args.control:
        ...
        from augmentation import in_train_control
