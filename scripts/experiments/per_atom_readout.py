"""The extensivity caveat: does the headline survive a size-normalised metric?

``||T||_F`` is computed from a per-structure tensor that the model builds by summing over atoms,
so it is extensive: bigger crystals give bigger violations, and a fixed absolute threshold (0.01
C/m²) is therefore size-dependent. The main analysis reports a Spearman rho of ~0.5-0.7 between
violation and atom count.
...
and touches nothing in ``results/stats.json``.
    ...
    uv run --extra nequip python scripts/experiments/per_atom_readout.py
"""
...
OUT_JSON = REPO / "results" / "per_atom_readout.json"
OUT_MD = REPO / "docs" / "results" / "per_atom_readout.md"
...
def main() -> None:
    ...
    lines = [
        "# The extensivity caveat, in numbers",
        "",
