"""Test-time inversion averaging: what symmetrising an SO(3) model's output actually buys.

The natural post-hoc fix is the odd projection ``T_sym(x) = [T(x) - T(I.x)] / 2``, which
symmetrises an SO(3) model's output at test time.

For an exactly equivariant model this is vacuous on *both* OOD variants, and provably so for
either arm: if ``x`` is centrosymmetric then ``I.x`` is the same periodic crystal up to a
permutation of atoms and a translation, so any permutation- and translation-invariant model
returns ``T(I.x) = T(x)`` and ``T_sym == 0`` identically. Measurement confirms this even on the
raw (DFT-relaxed) variant, whose coordinates satisfy inversion only to within a tolerance -- we
expected a residual signal there and there is none. Symmetrisation removes the false flags, for a
reason that carries no information about parity.
...
On an O(3) arm the same projection is a *correctness check*: the model already satisfies ``T(I.x)
= -T(x)``, so ``T_sym == T``. That law is verified on non-centrosymmetric crystals, where ``T`` is
a real signal -- on centrosymmetric input it is machine noise and no identity is measurable.

Run per profile, then render:

    uv run --extra nequip python scripts/experiments/inversion_averaging.py \
        --cores nequip allegro equiformer_v2
    uv run --extra mace   python scripts/experiments/inversion_averaging.py --cores mace
    python scripts/experiments/inversion_averaging.py --render
"""
...
OUT_JSON = REPO / "results" / "inversion_averaging.json"
OUT_MD = REPO / "docs" / "results" / "inversion_averaging.md"
