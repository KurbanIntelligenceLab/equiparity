"""The parity guarantee is differentiable: J = dT/dr at a centrosymmetric geometry.

For an O(3) model ``T(I.x) = -T(x)``. At a centrosymmetric ``x0`` (fixed by inversion up to the
atom permutation sigma) define ``P`` on displacements by ``(Pu)_i = -u_{sigma(i)}``.
Differentiating gives

    J . P = -J

so every inversion-*even* displacement lies in ``ker J``: the learned response to symmetry
breaking is supported entirely on inversion-odd modes, which are exactly the polar
(ferroelectric-like) modes. The guarantee is not just a zero -- it has physically structured
derivatives.
...
Every Jacobian is spot-checked against central finite differences before it is used.
    ...
    uv run --extra nequip --extra data python scripts/experiments/jacobian.py --cores nequip allegro
    uv run --extra mace --extra data python scripts/experiments/jacobian.py --cores mace
    python scripts/experiments/jacobian.py --render
"""
...
OUT_JSON = REPO / "results" / "jacobian.json"
OUT_MD = REPO / "docs" / "results" / "jacobian.md"
...
def render() -> None:
    ...
    lines = [
        "# The Jacobian of the parity guarantee",
        "",
    ...
    lines += [
        ...
        "forward pass is",
        "also stochastic (the output-parity audit). Recovering its Jacobian would require editing "
        "vendored ",
        "rotation code and",
