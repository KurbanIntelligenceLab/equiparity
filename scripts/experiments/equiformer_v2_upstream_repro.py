"""Reproduce the EquiformerV2 findings on upstream source (@ 8fe8cba), with no OCP install.

Builds a model from ``third_party/equiformer_v2_shimmed/`` (upstream verbatim + the documented
OCP-removal edits; see ``equiformer_v2_upstream_build.py``), with ``generate_graph`` relocated out
of the model file and provided by a small, auditable shim subclass. Then:
...
1. **Equivalence** — the shimmed-upstream model reproduces the shipped vendored model
   **bit-for-bit**
   ...
   behaviour verbatim).
2. **Pass-through bit-identity (Condition 2 by measurement)** — the output-parity and
   Jacobian measurements are
   **bit-identical**
...
3. **Reruns** — output-parity determinism (single-draw + 5-seed average), rotation,
   mirror; the Jacobian FD-vs-autograd —
   on the shimmed-upstream model, compared to the vendored measurements.

Run: ``uv run --extra nequip python scripts/experiments/equiformer_v2_upstream_repro.py``
"""
...
OUT_JSON = REPO / "results" / "equiformer_v2_upstream.json"
...
def _trained_wrapper(model, dev):
    """Wrap a shimmed model as a reload-harness TrainedModel so the output-parity and Jacobian
...
def _eqv2_fd_vs_autograd(model, structure, dev, seed: int = 4242) -> float:
    ...
    edge_distance_vec from a differentiable ``pos`` at fixed connectivity (the Jacobian patch).
    """
    from equiparity.models.equiformer import to_pyg_data
...
def _reruns(cfg, irreps) -> dict:
    """Output-parity determinism/rotation/mirror + Jacobian FD on the shimmed-upstream
    model, trained weights."""
    sys.path.insert(0, str(REPO / "scripts" / "experiments"))
    from output_parity import _random_rotation, _rel_error, _transform, _wigner
    ...
    runs = find_piezo_runs(
        Path(os.environ.get("PARITY_RUNS", Path(__file__).resolve().parents[2] / "runs")),
    ...
    # match the vendored the output-parity determinism check, which used the first 5 centrosymmetric OOD structures.  # noqa: E501
    ood = CrystalDataset(
        ...
        rows.append(
            {
                ...
                "jacobian_fd_rel_error": fd,
            }
        )
        r = rows[-1]
        print(
            f"[3] {label:38s} mirror={r['mirror_rel_error']:.3f} rot={r['rotation_rel_error']:.3f} "
            f"det={r['determinism_spread']:.3f} FD={r['jacobian_fd_rel_error']:.2f}"
        )
    return {"rows": rows}
