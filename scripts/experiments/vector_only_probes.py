"""Reflection-probe the three vector-only prevalence-audit rows.

Declared prediction (before measurement): the single Cartesian vector channel of PaiNN,
TorchMD-Net (ET) and ViSNet is built from edge vectors and is therefore implicitly POLAR -- under
any improper operation g the node vector features satisfy X(g.x) = g X(x) (not the pseudovector
law det(g) g X(x)). If so, these architectures carry parity implicitly while typing nothing, and a
correctly built rank-3 readout (odd number of vector factors) would inherit the structural zero --
unlike the SO(3)-only rows, which no readout can repair.
...
* **PaiNN** -- the audit row's pinned evidence object is torchmd-net's shared vector channel;
  ...
  an ephemeral env (``--with-schnetpack``, uses ``uv run --with schnetpack``).
    ...
    uv run python scripts/experiments/vector_only_probes.py --with-schnetpack
"""
