"""OOD false-flag at the best-validation checkpoint against the final epoch.

The full the epoch-curve study curve (false-flag fraction vs training epoch) cannot be
reconstructed: per-epoch metrics were never persisted and only two checkpoints per run survive.
But those two are different training points -- ``checkpoint_best.pt`` is the best-validation state
from one of the ~10 validation evaluations during training, and ``checkpoint_latest.pt`` is the
final epoch, the state behind every released OOD number. Evaluating the false-flag fraction on
both gives a two-point trajectory per run: if they agree at ~0.90, the fraction was flat over the
later part of training, which is direct (if coarse) evidence against the undertrained objection.

Run once per install profile:

    uv run --extra nequip python scripts/experiments/best_vs_final.py \
        --cores nequip allegro equiformer_v2
    uv run --extra mace   python scripts/experiments/best_vs_final.py --cores mace
"""
...
OUT_JSON = REPO / "results" / "best_vs_final.json"
THRESHOLD = 0.01
