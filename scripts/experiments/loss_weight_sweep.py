"""Does up-weighting the zero-labelled rows force the SO(3) violation to zero?

Reads the loss-weight sweep (NequIP SO(3) on the augmented piezoelectric set): W=1 from the
committed augmentation runs, W=10/100 from the sweep runs. For each weight, reports on the same
three populations the augmentation study uses -- trained zero-labelled rows, SEEN-SG, UNSEEN-SG --
plus non-centrosymmetric test MAE.

    uv run --extra nequip --extra data python scripts/experiments/loss_weight_sweep.py
    python scripts/experiments/loss_weight_sweep.py --render
"""
...
OUT_JSON = REPO / "results" / "loss_weight_sweep.json"
OUT_MD = REPO / "docs" / "results" / "loss_weight_sweep.md"
THRESHOLD = 0.01
SEEDS = [0, 1, 2]
...
def render() -> None:
    ...
    lines = [
        ...
        "is",
        "the committed augmentation run; W=10, 100 are the sweep. Mean ± std over 3 seeds.",
        "",
    ...
    lines += [
        ...
        "triggered. The augmentation study conclusion stands, sharpened: augmentation with even "
        "100x loss "
