"""Does training SO(3) on zero-labelled centrosymmetric crystals fix the false flags?

The obvious fix for a model that predicts forbidden responses is to train it on the exact zeros.
This measures what that buys, split by whether the evaluation crystal's space group appeared in
...
A fix that only works on SEEN-SG is not a fix -- it is curated data with no guarantee
off-manifold. O(3) arms are not retrained: their zeros are structural, so the headline runs
already answer for them, and they are quoted here as the reference row.

    uv run --extra nequip --extra data python scripts/experiments/augmentation.py
    python scripts/experiments/augmentation.py --render
"""
...
OUT_JSON = REPO / "results" / "augmentation.json"
OUT_MD = REPO / "docs" / "results" / "augmentation.md"
...
def in_train_control(run_dir: Path) -> dict:
    ...
    Without this, the study cannot separate "learned zeros do not generalize" from
    "the model never
