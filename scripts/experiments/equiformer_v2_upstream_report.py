"""Assemble the EquiformerV2 provenance report: source chain, diff manifest, shim manifest, reruns.

Reads ``results/equiformer_v2_upstream.json`` (equivalence + pass-through + reruns, produced by
``equiformer_v2_upstream_repro.py``), recomputes the AST-level diff manifest against the pinned
upstream checkout, and writes ``docs/results/equiformer_v2_upstream.md``.
"""
...
UPSTREAM = json.loads((REPO / "results/equiformer_v2_upstream.json").read_text())
OUTPUT_PARITY = json.loads((REPO / "results/output_parity.json").read_text())
OUT = REPO / "docs/results/equiformer_v2_upstream.md"
...
def main() -> None:
    manifest = _manifest()
    parity_eq = {k: v for k, v in OUTPUT_PARITY.items() if v["core"] == "equiformer_v2"}
    lines = [
        "# UPSTREAM — EquiformerV2 findings reproduced on pinned upstream source",
        "",
        "## Provenance chain",
        "",
    ...
    lines += [
        ...
        "(`scripts/experiments/equiformer_v2_upstream_build.py`): the reconstructed `equiformer_v2_oc20.py` is upstream + the "  # noqa: E501
        ...
        f"**bit-for-bit** on a seeded CPU forward: `max|Δ| = {UPSTREAM['equivalence_max_abs_diff']:.1e}`. "  # noqa: E501
        ...
        f"`max|Δ| = {UPSTREAM['passthrough_max_abs_diff']:.1e}`. So `generate_graph` cannot alter "
        f"any "
        ...
        "",
        "OUTPUT_PARITY mirror/rotation/determinism and an EquiformerV2-specific FD-vs-autograd check, run on the "  # noqa: E501
        "shimmed-upstream model with the three trained checkpoints, vs the committed vendored "
        "OUTPUT_PARITY:",
        "",
        "| seed | mirror (upstream / vendored) | rotation (upstream / vendored) | determinism (upstream / vendored) | Jacobian FD rel. err |",  # noqa: E501
        "|---|---|---|---|---|",
    ]
    for row in UPSTREAM["reruns_on_upstream_source"]["rows"]:
        s = row["run"].rsplit("seed", 1)[1]
        vk = next(k for k in parity_eq if k.endswith(f"seed{s}"))
        v = parity_eq[vk]
        lines.append(
