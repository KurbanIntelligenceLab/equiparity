"""Two further polar distortion paths, so rutile TiO2 does not carry the result alone.

Both parents were chosen for the 432 lesson from the symmetry-breaking sweep: their point group is
4/mmm, whose proper-rotation subgroup 422 admits a rank-3 invariant, so at delta = 0 only parity
forbids a piezoelectric response and the O(3)/SO(3) arms must separate exactly as they do on
rutile.
...
Every frame is spglib-verified at symprec 1e-8 before prediction, as in the symmetry-breaking
sweep. Same sweep grid, same models, same draws.

Run once per install profile:

    uv run --extra nequip python scripts/experiments/frozen_backbone_distortion.py \
        --cores nequip allegro equiformer_v2
    uv run --extra mace   python scripts/experiments/frozen_backbone_distortion.py --cores mace
"""
...
OUT_CSV = REPO / "results" / "frozen_backbone_distortion.csv"
OUT_JSON = REPO / "results" / "frozen_backbone_distortion.json"
OUT_MD = REPO / "docs" / "results" / "frozen_backbone_distortion.md"
...
# Same grid as the symmetry-breaking sweep: log-spaced tail + linear body.
DELTAS = np.unique(
...
def render() -> None:
    ...
    lines = [
        "# Additional polar distortion paths (SnO2 rutile-type, anatase TiO2)",
        "",
