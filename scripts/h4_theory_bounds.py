"""H4 -- quantitative test of Proposition 1 and the stability clause of Corollary 3.

Proposition 1 (arithmetic floor): on a centrosymmetric x, with
``delta = ||f(I.x) - f(x)||`` (failure of A1-A2) and ``eta = ||f(I.x) + f(x)||`` (failure of the
O(3) law at Q = I), every model satisfies ``||f(x)|| <= (delta + eta)/2``. Measured here as
absolute quantities per structure over the full 2,000-crystal idealized evaluation population,
for every arm.

Corollary 3 (rotation ceiling, stability clause): on class m-3m, with
``delta_plus = max_R ||f(R.x) - f(x)||`` and
``epsilon = max_R ||f(R.x) - D(R) f(x)|| / ||f(x)||`` over the proper-rotation subgroup
G+(x) = 432, an approximately SO(3)-equivariant model satisfies
``||f(x)|| <= delta_plus / (1 - epsilon)`` wherever ``epsilon < 1``. Measured over the full
proper subgroup (spglib rotations of the idealized structure, converted to Cartesian) on the
166 m-3m crystals, for the SO(3) arms the clause bounds.

Run once per install profile:

    uv run --extra nequip python scripts/h4_theory_bounds.py \
        --cores nequip allegro equiformer_v2
    uv run --extra mace   python scripts/h4_theory_bounds.py --cores mace
...
from e4_inversion_averaging import _invert, _norms
...
OUT_MD = REPO / "docs" / "results" / "h4_theory_bounds.md"
