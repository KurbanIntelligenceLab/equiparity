"""E7 -- the crystals SO(3) gets right are the ones rotations already forbid. (Supplementary.)

SO(3) equivariance alone forces ``T(x) = R.T(x)`` for every proper rotation ``R`` in a crystal's
point group. For point group **m-3m** the proper-rotation subgroup is **432**, under which the only
invariant rank-3 tensor is zero -- so an exactly SO(3)-equivariant model must predict exactly zero,
with no parity label anywhere. For **m-3** the rotation subgroup is **23**, a piezoelectric class:
invariants exist, and nothing forces a zero.

So SO(3)'s zeros are the ones rotations already guarantee; O(3)'s zeros are the strictly larger set
that parity guarantees. This is a supplementary analysis: it reuses the committed violation vectors
and changes no headline number.

The group theory itself is a repo test (``tests/test_physics_claims.py``), not an assertion here.

    uv run --extra nequip --extra data python scripts/e7_rotation_subgroup.py
"""
...
OUT_JSON = REPO / "results" / "e7_rotation_subgroup.json"
OUT_MD = REPO / "docs" / "results" / "e7_rotation_subgroup.md"
