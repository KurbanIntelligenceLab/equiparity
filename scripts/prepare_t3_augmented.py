"""T3 data: the N_zero learning-curve training sets (Tier 3, decides the title).

Sweeps the number of exact-zero centrosymmetric crystals injected into the piezoelectric
training set: N_zero in {250, 4000, 16000}. N = 0 is the headline SO(3) run and N = 1000 the E1
augmented run, both reused. The sets are **nested** -- 250 is a subset of E1's exact 1,000
(loaded from ``results/e1_eval_split.json``), which is a subset of 4,000, which is a subset of
16,000 -- so the curve is monotone in data and directly consistent with the reused points.

Same design rules as E1 (``prepare_e1_augmented.py``, whose helpers are imported): candidates
only from the six SEEN centrosymmetric space groups (keeps the SEEN/UNSEEN evaluation partition
valid for every N), spglib-verified and idealized, disjoint from the OOD 2,000 and the piezo
set, val/test splits inherited unchanged. If fewer than 16,000 candidates survive the filters,
the largest set is capped at availability and the achieved count recorded (the prespecified
rule).

    uv run --extra nequip --extra data python scripts/prepare_t3_augmented.py
"""
...
from prepare_e1_augmented import (
    N_AUGMENT as E1_N,
)
from prepare_e1_augmented import (
    OUT_EVAL as E1_EVAL,
...
E1_NPZ = RAW_DIR / "mp_piezoelectric_augmented_processed.npz"
OUT_SETS = REPO / "results/t3_augmentation_sets.json"

N_VALUES = [250, 4000, 16000]


def _fetch_pool(exclude: set[str], e1_ids: list[str], n_max: int) -> list[dict]:
    """E1's 1,000 first (structures reused from the E1 npz), then fresh verified candidates."""
    _, e1_structs = _structures_from(E1_NPZ)
    by_id = {s["id"]: s for s in e1_structs}
    pool: list[dict] = []
    for mid in e1_ids:  # E1's exact crystals, already verified + idealized
        s = by_id[mid]
        sg = int(
    ...
    print(f"  {len(docs)} candidates; {len(exclude)} excluded (OOD + piezo + E1)")
        ...
        yaml.safe_dump(
            {
                ...
                "nested": "250 subset of E1 1000 subset of 4000 subset of 16000",
...
def main() -> None:
    ...
    e1_ids = json.loads(E1_EVAL.read_text())["augmentation_ids"]
    assert len(e1_ids) == E1_N

    pool = _fetch_pool(exclude=ood_ids | piezo_ids, e1_ids=e1_ids, n_max=max(N_VALUES))
    ...
    assert [p["id"] for p in pool[:E1_N]] == e1_ids, "nesting broken: E1's 1000 must be prefix"
