...
Space groups are not stored in the processed npz, and both E6 (named materials) and E7 (the
rotation-subgroup analysis) need them per structure. Computed once with spglib at the same
tolerance used to build the set (``symprec = 1e-3``) and cached.

    uv run --extra nequip --extra data python scripts/ood_spacegroups.py
"""
